            ...
            <div class="ibox">
                ...
                <div class="ibox-content">
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover nowrap" id="restaurant-types-table"
                            width="100%">
                            <thead>
                                <tr>
                                    {{-- <th>{{__('No.')}}</th> --}}
                                    <th>{{__('Restaurant Type')}}</th>
                                    <th>{{__('Status')}}</th>
                                    <th>{{__('Action')}}</th>
                                </tr>
                            </thead>
...
<script>
    var restaurantTypesTable = $("#restaurant-types-table").DataTable({
    ...
    $(document).on('click', '.deleteRecord', function(event) {
        ...
        var deleteMessage = "{{ __('You want to delete this restaurant type?') }}";
        var isDelete = deleteRecordByAjax(deleteUrl, "{{$module_name}}", restaurantTypesTable, null, deleteMessage);
    });
