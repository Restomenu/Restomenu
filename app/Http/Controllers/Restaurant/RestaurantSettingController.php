...
{
    public function __construct(RestaurantTime $model, Setting $settingModel, Restaurant $restaurantModel)
    {
    ...
    {
        $restaurant = $this->restaurantModel->where('email', $request->email)->get();
        return (($restaurant->count() > 0) ? "false" : "true");
    }

    public function checkUniqueSlug(Request $request)
    {
        $restaurant = $this->restaurantModel->where('slug', $request->slug)->get();
        return (($restaurant->count() > 0) ? "false" : "true");
    }
}
